 ...
 * todo:add optimization: for more ram to fast program when k is big
 */
public class DiffFPM extends FSG.FSG {
    private double e1,e2;
    private double ita1,ita2;
    private Random rSeed;
    private int nConThrehsold;
    HashSet<String> hVerLabel;                                          //iejr: all vertexs
    HashMap<String,GraphSet> hCanLabel2SubOneGraph;                     //iejr: all edges
    public HashSet<String> hFilterLabel;                                       //iejr: the graph which has been finded
    public HashMap<String,GraphSet> hCanLabel2FreGraph;                        //iejr: all frequent graphs and their canonical labels

    protected HashMap<String,Neighbors> hLabel2Neighbor;
    protected HashMap<String,GraphSet> hLabel2NeighborGraph;
    protected int nUniNeighborID;
    ...
    public static void main( String[] args ){
        ...
        DiffFPM.testThresholdMining( Dataset, k, threshold,dbSize, epsilon1, epsilon2 );
    ...
    public DiffFPM(){
        ...
        this.hLabel2Neighbor = new HashMap<String,Neighbors>();
        this.hLabel2NeighborGraph = new HashMap<String,GraphSet>();
        this.nUniVerID = 0;
    }
            ...
            while( (sLine = r.readLine()) != null ) {
                ...
                for( GraphSet gGraph : hCanSubOneGraph ){
                    if( !hCanLabel2SubOneGraph.containsKey( gGraph.sCanLabel ) ) {
                        ...
                        hCanLabel2SubOneGraph.get( gGraph.sCanLabel ).hTidList.addAll( aUpdateList );
                    }
                }
    ...
    public ArrayList<Neighbors> EEN( GraphSet gGraphPattern, HashSet<GraphSet> hPositive, HashSet<GraphSet> hNegtive, HashSet<GraphSet> hHalfPositive ){
    ...
    //    System.out.println( "Current graph is: " + gGraphPattern.sCanLabel );
        ...
        HashSet<GraphSet> hSubNeighbors = lTestGraph.getSubNeighbors( this.hFilterLabel, hOccursInFilters );
     //   System.out.println("The sub graph set has been populated : " + hSubNeighbors.size());

        HashMap<GraphSet,LabelEdge> hSuperBackNeighbors = lTestGraph.getSuperBackNeighbors( this.hCanLabel2SubOneGraph, this.hFilterLabel, hOccursInFilters );
    //    System.out.println( "The super back set has been populated : " + hSuperBackNeighbors.size() );

        HashMap<GraphSet, LabelEdge> hSuperForwardNeighbors = lTestGraph.getSuperForwardNeighbors( this.hVerLabel, this.hCanLabel2SubOneGraph, this.hFilterLabel, hOccursInFilters );
    //    System.out.println( "The super forward set has been populated : " + hSuperForwardNeighbors.size() );
        ...
        //    hNegtive.addAll( hSuperBackNeighbors.keySet() );
            ...
            countSuperBackPattern( gGraphPattern, hSuperBackNeighbors, hTid2VerMap, hPositive, hNegtive );
        }
        ...
        //    hNegtive.addAll( hSuperBackNeighbors.keySet() );
            ...
            countSuperForwardPattern(gGraphPattern, hSuperForwardNeighbors, hTid2VerMap, hPositive, hNegtive);
        }
    ...
    //    hOccursInFilters.clear();
        ...
        HashSet<Integer> hNeighborSelected = new HashSet<Integer>();
        ArrayList<Neighbors> aNeighbor = new ArrayList<Neighbors>();
        ...
        //    System.out.print( "The sub graph set has been populated : " + hSubNeighbors.size() );

            hSuperBackNeighbors = lTestGraph.getSuperBackNeighbors( this.hCanLabel2SubOneGraph, null,null );
        //    System.out.println( "The super back set has been populated : " + hSuperBackNeighbors.size() );

            hSuperForwardNeighbors = lTestGraph.getSuperForwardNeighbors( this.hVerLabel, this.hCanLabel2SubOneGraph, null,null );
        //    System.out.println( "The super forward set has been populated : " + hSuperForwardNeighbors.size() );
            ...
            */
            ...
            Neighbors nNeighbor = this.hLabel2Neighbor.get( sCanLabel );

            if( !hNeighborSelected.contains( nNeighbor.nID ) ){
                hNeighborSelected.add( nNeighbor.nID );
                aNeighbor.add( nNeighbor );

                for( String sCanLabelNeighbor : nNeighbor.hPositive ){
                    if( hCandidateCanLabel.contains( sCanLabelNeighbor ) || sCanLabelNeighbor.equals( gGraphPattern.sCanLabel ) ){
                        continue;
                    }
                    GraphSet gGraph = this.hLabel2NeighborGraph.get( sCanLabelNeighbor );
                    hPositive.add( gGraph );
                }
                for( String sCanLabelNeighbor : nNeighbor.hHalfPositive ){
                    if( hCandidateCanLabel.contains( sCanLabelNeighbor ) || sCanLabelNeighbor.equals( gGraphPattern.sCanLabel )  ){
                        continue;
                    }
                    GraphSet gGraph = this.hLabel2NeighborGraph.get( sCanLabelNeighbor );
                ...
                for( String sCanLabelNeighbor : nNeighbor.hNegtive ){
                    if( hCandidateCanLabel.contains( sCanLabelNeighbor ) || sCanLabelNeighbor.equals( gGraphPattern.sCanLabel )  ){
                        continue;
                    }
                    GraphSet gGraph = this.hLabel2NeighborGraph.get( sCanLabelNeighbor );
    ...
    protected void countPattern( HashSet<GraphSet> hGraphPattern, HashSet<GraphSet> hHalfPositive, HashSet<GraphSet> hNegtive ){
        if( hGraphPattern == null || hGraphPattern.size() == 0 ){
            return;
        }

        //iejr: for each sub graph, find all edges and count the intersaction of their tid list
        int nNegtiveCount = 0;
        for( GraphSet gGraph : hGraphPattern ){
            ...
            for( String sCanLabel : hGraphCanLabel ){
                ...
                //    System.out.println( "Error! no edge matched in the global domain!" + sCanLabel );
                //    System.exit( 0 );
    ...
    //    System.out.println( "|C|: " + hGraphPattern.size() + "  |CN|: " + nNegtiveCount );
...
/*
        ...
        try{
            ...
            while( (sLine = r.readLine()) != null ) {
                ...
                //        System.out.println( "Jump!" );
                    ...
                    if( hGraph2Support.containsKey( gCanGraph ) && hGraph2Support.get( gCanGraph ) > this.dMinsup ){
                        continue;
                    }
    ...
    protected void countSubPattern( GraphSet X, HashSet<GraphSet> hGraphPattern, HashSet<GraphSet> hPositive, HashSet<GraphSet> hNegtive ){
        if( hGraphPattern == null || hGraphPattern.size() == 0 ){
            return;
        }
        ...
        try{
            ...
            while( (sLine = r.readLine()) != null ) {
                ...
                for( GraphSet gCanGraph : hGraphPattern ){
                    ...
                    if( X.hTidList.contains( nTid ) || gCanGraph.lGraph.judgeIsomorphismPrun( lGraph ) != null ){
                        if( hGraph2Support.containsKey( gCanGraph ) ){
                            int nSupport = hGraph2Support.get( gCanGraph ) + 1;
                            hGraph2Support.put( gCanGraph, nSupport );
                        }else {
    ...
    protected void countSuperBackPattern(
            GraphSet X, HashMap<GraphSet,LabelEdge> hSuperBackGraphPattern, HashMap<Integer,HashSet<int[]>> hTid2Map,
            ...
            while( (sLine = r.readLine()) != null ) {
                ...
                for( GraphSet gCanGraph : hSuperBackGraphPattern.keySet() ){
                    ...
                    for( int[] nVer2VerMap : hVer2VerMap ){
                        ...
                        if( lGraph.judgeEdge(nSourMappedVerID, sEndLabel, hFilter ).contains(dEdgeLabel) ){
                            if( hGraph2Support.containsKey( gCanGraph ) ){
                                int nSupport = hGraph2Support.get( gCanGraph ) + 1;
                                hGraph2Support.put( gCanGraph, nSupport );
                            }else {
    ...
    public ArrayList<GraphSet> mainDiffFPM( int k, String sRandomSelectPath, String sOutputPath ){
        ...
        while( aResult.size() < k ){
            ...
            ArrayList<Neighbors> aNeighborX = null;
            ...
            while( true ){
                ...
                if( nStateCnt > 200 ){
                    ...
                    updateNeighbor( gCurrentGraph, hPositiveX, hNegitiveX, aNeighborX );
                    double dNoisySupport = getNoisySupport( nSupportX, k );
                    writeFrequentGraph( gCurrentGraph, sOutputPath, dNoisySupport, (double)nSupportX );
                    break;
                }
                ...
                if( gOrgGraph != gCurrentGraph ) {
                    ...
                    aNeighborX = this.EEN(gCurrentGraph, hPositiveX, hNegitiveX, hHalfPositiveX);
                }
                ...
                Integer nCurrentNeighborNum = hPositiveX.size() + hNegitiveX.size() + hHalfPositiveX.size();
                ...
                System.out.println( "Iterat : " + nStateCnt + "    " + gOrgGraph.sCanLabel + "    " + gNextGraph.sCanLabel + "   " + bIsTransform );
            //    System.out.println( "         Support:  " + nSupportX + "  " + nSupportY );
                ...
                if( nConvergenceStateCnt >= this.nConThrehsold ||
                        (gCurrentGraph.lGraph.getRank() > 15 && hPositiveX.size() == 0 && hHalfPositiveX.size() == 0)
    ...
    protected void updateNeighbor(  GraphSet gCurrent, HashSet<GraphSet> hPositive, HashSet<GraphSet> hNegative, ArrayList<Neighbors> nNeighbor ){
    //    this.hFilterLabel.add( gCurrentGraph.sCanLabel );
    //    this.hCanLabel2FreGraph.put( gCurrentGraph.sCanLabel,gCurrentGraph );

        Neighbors nNew = new Neighbors( this.nUniNeighborID++ );
        nNew.hCore.add( gCurrent.sCanLabel );
        for( GraphSet gGraph : hPositive ){
            nNew.hPositive.add( gGraph.sCanLabel );
            this.hLabel2NeighborGraph.put( gGraph.sCanLabel, gGraph );
        //    this.hLabel2Neighbor.put( gGraph.sCanLabel, nNew );
        }
        for( GraphSet gGraph : hNegative ){
            nNew.hNegtive.add( gGraph.sCanLabel );
            this.hLabel2NeighborGraph.put( gGraph.sCanLabel, gGraph );
        //    this.hLabel2Neighbor.put( gGraph.sCanLabel, nNew );
        }

        this.hLabel2Neighbor.put( gCurrent.sCanLabel, nNew);

        if( nNeighbor.size() > 0 ) {
            Neighbors nCore = nNeighbor.get(0);

            for (int i = 1; i < nNeighbor.size(); i++) {
                Neighbors N = nNeighbor.get(i);
                nCore.merge(N);
            ...
            for( String sCanLabel : nCore.hCore ){
                this.hLabel2Neighbor.put(sCanLabel, nCore);
            }
    ...
    //    int nOffset  = 3;
        ...
        String sRandomSampleFilePath = sRandomSelectPath + "/" + nRandomGraphIndex + ".txt";
...
/*
    ...
    protected boolean judgeConvergence( ArrayList<Integer> aStateList ){
        ...
        double dFrontPro = 0.1;             //iejr: default value is 0.1
    ...
    protected void writeFrequentGraph( GraphSet gGraph, String sFilePath, double dNoisySupport, double dRealSupport ){
    ...
    public void testEEN(){
        ...
        System.out.println( "All edge patterns are  : " + myDFPM.hCanLabel2SubOneGraph.size() );
/*
    ...
    //iejr: no use
    public static void testTopKMining( String sDataset, int k,  double epsilon1, double epsilon2 ){


        DiffFPM myDFPM = new DiffFPM();
        ...
        double ita1 = 0.8;
        double ita2 = 0.1;
        int nRandomSeed = Parameter.nRandomSeed;       //iejr: -1 for random and other for pseudo random sequence
        int nConvergenceThreshold = 20;
        String sDatasetPath = DatasetParameter.getDatasetPath( sDatasetName );
        String sRandomStartPath = DatasetParameter.sDataResultPrefix + sDatasetName + "/FSG/badsample";
        String sOutputPath = DatasetParameter.sDataResultPrefix + sDatasetName + "/DiffFPM/top/" + "Result_top" + k + "_" + e1 + "_" + sTime + ".txt";
        String sFilterPath = DatasetParameter.sDataResultPrefix + sDatasetName + "/DiffFPM/filter.txt";
        ...
        myDFPM.setParameter( sDatasetPath, dSupport, e1, e2, ita1, ita2, nRandomSeed, nConvergenceThreshold );

        long nStart = System.currentTimeMillis();

        myDFPM.preProcessing();
        myDFPM.InitFilter(sFilterPath);
        myDFPM.mainDiffFPM( k, sRandomStartPath, sOutputPath );

        long nEnd = System.currentTimeMillis();
        System.out.println( "Time consuming : " + (nEnd - nStart) );
        myDFPM.writeTime( nEnd-nStart, sOutputPath );

        String sStand = DatasetParameter.sDataResultPrefix + sDatasetName + DatasetParameter.getContrastFile( sDatasetName );
        Contrast.ContrastSmart(sOutputPath, sStand, k);
    }


    public static void testThresholdMining( String sDataset, int k, double dThreshold, int nDatasize, double epsilon1, double epsilon2 ){
        ...
        double e1 = epsilon1;
        double e2 = epsilon2;
        double ita1 = 0.8;          //iejr: argument suggest by original paper, don't modify
        double ita2 = 0.1;          //iejr: argument suggest by original paper, don't modify
        int nRandomSeed = Parameter.nRandomSeed;       //iejr: -1 for random and other for pseudo random sequence
        int nConvergenceThreshold = 20;
        String sDatasetPath = "./Dataset/" + sDatasetName + ".dat";
        String sRandomStartPath = "./out/" + sDatasetName + "/FSG/badsample";
        String sOutputPath = "./out/" + sDatasetName + "/DiffFPM/threshold/" + "Result_th" + dThreshold + "_" + e1 + "_" + sTime + ".txt";
        String sFilterPath = "./out/" + sDatasetName + "/DiffFPM/filter.txt";
        ...
        String sStand = "./out/" + sDatasetName + "/FSG/" + "Result_" + dThreshold + "_" + "Standard" + ".txt";
        Contrast.ContrastSmart(sOutputPath, sStand, -1);
    }


    public static double testPhase2( String sFilePath, double e2 ){
...
class Neighbors{
    ...
    public HashSet<String> hPositive;
    public HashSet<String> hNegtive;
    public HashSet<String> hHalfPositive;

    public HashSet<String> hCore;
    ...
    public Neighbors(){
        ...
        hNegtive  = null;
        hHalfPositive = null;
        hCore = new HashSet<String>();
    }

    public Neighbors( int nID ){
        this.nID = nID;
        hPositive = new HashSet<String>();
        hNegtive  = new HashSet<String>();
        hHalfPositive = new HashSet<String>();
        hCore = new HashSet<String>();
    }

    public void merge( Neighbors nOther ){
        this.hPositive.addAll( nOther.hPositive );
        this.hHalfPositive.addAll( nOther.hHalfPositive );
        this.hNegtive.addAll( nOther.hNegtive );

        this.hCore.addAll( nOther.hCore );

        this.hPositive.removeAll( this.hCore );
        this.hHalfPositive.removeAll( this.hCore );
        this.hNegtive.removeAll( this.hCore );
    }

    public void clear(){
        this.nID = -1;
        this.hPositive.clear();
        this.hHalfPositive.clear();
        this.hNegtive.clear();
        this.hCore.clear();

        this.hPositive = null;
        this.hHalfPositive = null;
        this.hNegtive = null;
        this.hCore = null;
    }
}